...
In this section, we first describe our prototype implementation of the overall system (both the static analysis and the runtime synchronization algorithm), and then describe its experimental evaluation over a set of five real-world subjects. 

\subsection{Prototype System}

We have created a Java implementation of our static analysis for composed {\sf Map} operations, as described  in Section \ref{Se:instance}, which covers all the operations listed in Figure \ref{fig:language}. \pietrotodo{Move paragraph on $T_1,T_2,T_n$ over here.} 
...
\begin{itemize}
	...
	\item \underline{Concurrency level:} For a fixed workload size, set at \pengtodo{what is workload size?}, the number of threads is varied along the range of 1 to 16 threads.  
\end{itemize}
For statistical soundness, we report the average performance results across 10 independent repetitions of each experiments.

We compare our technique against two competing solutions:
\begin{itemize}
	\item version 1.3 of the Deuce STM (the latest version),\footnote{
		\url{https://github.com/DeuceSTM/DeuceSTM}
	} which implements a concrete-level pessimistic variant of STM; and
	\item a lock-based solution synchronization algorithm boosted with {\sf Map} semantics \cite{EricBoosting}.
\end{itemize}
