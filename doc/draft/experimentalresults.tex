...
Our benchmark suite consists of four subjects, all of which are taken from popular open-source code bases, as follows:
\begin{itemize}
	\item XXX
\end{itemize}
...
We depict the performance results in Figures \ref{XXX}-\ref{XXX}. For each benchmark, we present two graphs: The (a) graphs correspond to the first measurement, where we increase the size of the workload while keeping the number of threads fixed at 8. The expected trend for these graphs is monotonic increase in running time. The (b) graphs correspond to a fixed-size workload executed by each of the transactions, where the number of transactions increases. Here, too, we expect an increase in running time due to interference between threads.

Indeed, across all benchmarks, both the (a) and the (b) graphs largely follow the expected trend. Further, in all cases the warping approach achieves the lowest running time. The most significant gap is w.r.t. benchmark XXX, where the gap in completion time  increase in payload size is almost x5 and the diffe in completion time over 32 threads is almost x6.
