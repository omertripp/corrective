...
to the right.
Figure \ref{fig:concretesemantics} defines the concrete semantics.
For the most part, it formalizes the API specification of the corresponding Java method. Note that \statement{assert} is defined only on the states that satisfy the given Boolean conditions. \newpietro{$n$ represents a fresh concrete node in the semantics of \statement{putIfAbsent}}. In this way, the concrete semantics filters out only the states that might execute a branch of an \statement{if} or \statement{while} statement.
...
\newcommand\fixup{\hspace{-0.25in}}

\begin{figure*}[t]
\[
\begin{array}{lr}
\csemantics{S}{\statement{v = new\ Value()}, (e, m)}
=  (e[v \mapsto \freshNode(\statement{t})], m)& \fixup (\mathtt{new})\\
%%
\csemantics{S}{\statement{v = null}, (e, m)}
= (e[v \mapsto \{\statement{null}\}], m)& \fixup (\mathtt{nlas})\\
%%
\csemantics{S}{\statement{v=m.get(k)}, (e, m)}
= (e[\statement{v} \mapsto \bigcup_{n \in e(k)} m(n)], m) & \fixup (\mathtt{get})\\
%%
  \csemantics{S}{\statement{m.put(k, v)}, (e, m)} & \fixup (\mathtt{put})\\
  \;\;\;=\left\{
\begin{array}{ll}
...
\right. & \\
%%
\csemantics{S}{\statement{m.remove(k)}, (e, m)} & \fixup (\mathtt{rmv})\\ 
\;\;\;=  \left\{
\begin{array}{ll}
...
\right. & \\
%%
\csemantics{S}{\statement{v = m.putIfAbsent(k, v)}, (e, m)} & \fixup (\mathtt{pIA})\\
...
\right. & \\
%%
\csemantics{S}{\statement{assert(x==null)}, (e, m)} & \fixup (\mathtt{null})\\
\;\;\;= \left\{
\begin{array}{ll}
...
\right. & \\
%%
\csemantics{S}{\statement{assert(m.containsK(k))}, (e, m)} & \fixup (\mathtt{cntK})\\
\;\;\;= \left\{
\begin{array}{ll}
...
%\label{Se:concabs}
We apply the abstract semantics defined in Section \ref{se:instance} to compute suitable corrective targets. In particular, we need that these targets are reachable from the same \emph{entry state} through a \emph{serializable execution}. Therefore, we build a CFG that represents certain specific \emph{serialized} executions. In particular, we assume that we have $k$ distinct types of transactions, and we build up a serialized CFG that represents a serialized execution of \emph{at least} 2 instances of each type of transaction.
...
\runex Similarly to other synchronization approaches, such as transactional boosting \cite{ppopp/HerlihyK08} and foresight \cite{pldi/Gueta13}, applying corrective synchronization at the data-structure level requires a commutativity specification. In the case of concurrent maps, a simple and effective specification is in terms of the accessed key. Hence, for the 
\textsf{getConvertor} code, we build a serialized CFG where all the transactions share the same key \statement{name} (to induce potential conflicts). For the sake of presentation, we set \statement{create} to \statement{true}. The serialized CFG consists of the sequence of transactions $t_1; t_n^\star; t_2$, where $t_n^\star$ represents that the code of $t_n$ is inside a loop.
...
\noindent
We now prove that targets computed by $\warpdestination$ satisfy the premise of {\sf corr}.
