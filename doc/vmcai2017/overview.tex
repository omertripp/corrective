...
%
\blue{Note that the corrective actions above are of a general form, which is not limited to two threads. For any number of threads, the corrected state would have one privileged thread deciding the return value (i.e., the value of {\sf clobbered}) for all threads as well as what $\elevenoclock$ should be associated with in the \textsf{map}.} \textcolor{green}{Also note that the correction does \emph{not} directly modify the shared state; rather, the correction is made to a thread-local replica of the state. After the correction, if the thread is able to commit, then shared-state mutations are applied at commit time.} 
