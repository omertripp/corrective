...
Res updateReservation(Slot slot, Slot slot') {
  Res r = map.remove(slot);
  if (r == null)
    r = new Res();
  Res dead = map.replace(slot', r);
  return dead;
}
	\end{lstlisting}
...
\begin{center}
   ...
   [ & & $t_1$: ${\sf remove(\tenoclock)}/\alpha$ & $\rightarrow$ & $t_2$: ${\sf remove(\thirteenoclock)}/{\sf null}$  & \\
     & $\hookrightarrow$ & $t_1$: ${\sf if(\ldots)}$ & $\rightarrow$  & $t_2$: ${\sf new\ Res}()/\delta$ & \\
...
This history is clearly not serializable. \blue{$t_1$ cannot start first, yet return the effect ($\delta$) of a concurrent transation.}
%In any serializable history, $t_1$ and $t_2$ would return the same {\sf Convertor} instance.
However, we can warp to such a serializable history by applying a correction.
%
\blue{In this case, note that there are two possible serializable histories:
...
%
\blue{Note that the corrective actions above are of a general form, which is not limited to two threads. For any number of threads, the corrected state would have one privileged thread deciding the return value (i.e., the value of {\sf dead}) for all threads as well as what $\elevenoclock$ should be associated with in the \textsf{map}.} \textcolor{green}{Also note that the correction does \emph{not} directly modify the shared state; rather, the correction is made to a thread-local replica of the state. After the correction, if the thread is able to commit, then shared-state mutations are applied at commit time.} 
...
%
\newpietro{If the oracle was not able to compute a target for the correction (e.g., because of a loss of precision of the static analysis), then our approach can apply optimistic synchronization}
